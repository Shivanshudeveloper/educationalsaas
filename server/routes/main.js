...
router.post("/addclasssubject", async (req, res) => {
  const  {formData,userEmail} = req.body;
  try {
    const newClass = new ClassSubject_Model({...formData,userEmail:userEmail});
    await newClass.save();
...
router.post("/addclasslevel", async (req, res) => {
  const {formData,userEmail} = req.body;
  try {
    const newClass = new ClassLevel_Model({...formData,userEmail:userEmail});
    await newClass.save();
...
router.get("/getallresponses/:userEmail", async (req, res) => {
  try {
    const allClasses = await FormResponse_Model.find({userEmail:req.params.userEmail});
    res.status(201).json(allClasses);
  } catch (error) {
    res.status(404).json({ message: "Error" });
  }
...
router.get("/gettraininglist/:userEmail", async (req, res) => {
  try {
    const allClasses = await Training_Model.find({userEmail:req.params.userEmail});
    res.status(201).json(allClasses);
  } catch (error) {
    res.status(404).json({ message: "Error" });
  }
